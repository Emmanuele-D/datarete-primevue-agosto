    ...
    <div class="grid">
      <div class="col-12 col-lg-4">
        <Card>
          <template #content>
            ...
            <div v-else>
              ...
              <div>
                <div :class="{ 'not-valid-item': !valid_nomeCliente }" class="flex flex-column w-full mb-4">
                  <label>Nome e cognome</label>
                  <InputText type="text" v-model="objectToPost.nomeCliente"></InputText>
                </div>
                <div :class="{ 'not-valid-item': !valid_dataNascita }" class="flex flex-column w-full mb-4">
                  <label>Data di nascita</label>
                  <Calendar v-model="objectToPost.dataNascita"></Calendar>
                </div>
                <div :class="{ 'not-valid-item': !valid_dataAssunzione }" class="flex flex-column w-full mb-4">
                  <label>Data assunzione</label>
                  <Calendar v-model="objectToPost.dataAssunzione"></Calendar>
                </div>
                <div :class="{ 'not-valid-item': !valid_sesso }" class="flex flex-column w-full mb-4">
                  <label>Genere</label>
                  <Dropdown :filter="true" :options="genereOptions" optionLabel="text" optionValue="value"
                    v-model="objectToPost.sesso">
                  </Dropdown>
                </div>
                <div :class="{ 'not-valid-item': !valid_tipoRapporto }" class="flex flex-column w-full mb-4">
                  <label>Tipo rapporto</label>
                  <Dropdown :filter="true" :options="tipoRapportoOptions" optionLabel="text" optionValue="text"
        ...
        <Card class="mt-4">
          <template #content>
            <h2>Mutuo</h2>
            <div :class="{ 'not-valid-item': !valid_finalita }" class="flex flex-column w-full mb-4">
              <label>Finalità</label>
              <Dropdown :filter="true" :options="finalita" optionValue="value" optionLabel="text"
                v-model="objectToPost.finalita">
              </Dropdown>
            </div>
            <div :class="{ 'not-valid-item': !valid_tipoTasso }" class="flex flex-column w-full mb-4">
              <label>Tipo di tasso</label>
              <Dropdown :filter="true" :options="tipoTasso" optionValue="text" optionLabel="text"
                v-model="objectToPost.tipoTasso">
              </Dropdown>
            </div>
            <div :class="{ 'not-valid-item': !valid_redditoRichiedenti }" class="flex flex-column w-full mb-4">
            ...
            <div :class="{ 'not-valid-item': !valid_importoRichiesto }" class="flex flex-column w-full mb-4">
              <label>Importo richiesto</label>
              <InputNumber currency="EUR" mode="currency" locale="it-IT" :minFractionDigits="2"
                v-model="objectToPost.importoRichiesto"></InputNumber>
            </div>
            <div :class="{ 'not-valid-item': !valid_durataAnni }" class="flex flex-column w-full mb-4">
              <label>Durata</label>
              <Dropdown :filter="true" type="text" v-model="objectToPost.durataAnni" :options="durataOptions">
              </Dropdown>
            </div>
            <div :class="{ 'not-valid-item': !valid_provinciaImmobile }" class="flex flex-column w-full mb-4">
              <label>Provincia immobile</label>
              <InputText type="text" v-model="objectToPost.provinciaImmobile"></InputText>
            </div>
            <div :class="{ 'not-valid-item': !valid_valoreImmobile }" class="flex flex-column w-full mb-4">
              <label>Valore immobile</label>
              <InputNumber currency="EUR" mode="currency" locale="it-IT" :minFractionDigits="2"
                v-model="objectToPost.valoreImmobile"></InputNumber>
            </div>
            <div :class="{ 'not-valid-item': !valid_provvigione }" class="flex flex-column w-full mb-4">
            ...
            <template #content>
              ...
              <div class="flex justify-content-between">
                ...
                <div class=" flex flex-grow justify-content-between">
                  ...
                  <Button @click="openPreventivo(item)" label="Salva Preventivo"
...
function getRetail(id) {
  ...
  service.read()
    .then(res => {
      ...
      objectToPost.value.dataNascita = new Date(res.dataNascita).toLocaleDateString('it', {
        year: 'numeric',
        month: '2-digit',
        day: '2-digit',
      })
      objectToPost.value.dataAssunzione = new Date(res.dataAssunzione).toLocaleDateString('it', {
        year: 'numeric',
        month: '2-digit',
        day: '2-digit',
      })
...
function ricercaRisultati() {
  ...
  axios.post('https://prestitosi-core.datarete.cloud/api/Preventivatore/PreventivoMutuo', objectToPost.value)
    .then(response => {
