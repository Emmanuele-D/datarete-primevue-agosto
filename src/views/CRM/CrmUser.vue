    ...
    <Card v-if="currentCrm" class="mb-4">
      <template #content>
        ...
        <div class="my-2" v-if="currentStatoAnagrafica">
          <p class="m-0">Ultimo stato: <span class="font-bold">{{ currentStatoAnagrafica.STATO }}</span></p>
          <p v-if="currentStatoAnagrafica.ID_SOTTOSTATO != 0" class="text-gray-500 m-0">{{
    currentStatoAnagrafica.SOTTOSTATO
}}</p>
          <p v-if="currentStatoAnagrafica.DATA_STATO" class="text-gray-500 mb-4 font-italic">{{ new
    Date(currentStatoAnagrafica.DATA_STATO).toLocaleDateString('it')
          ...
          <div v-for="(item, index) in currentCrm.contatti" :key="index"
            class="flex w-full justify-content-start gap-4 cursor-pointer mb-2" @click="contattoAction(item)">

            <div>
              <i v-if="item.idTipoContatto == 1" class="pi pi-at"></i>
              <i v-if="item.idTipoContatto == 2 || item.idTipoContatto == 3" class="pi pi-phone"></i>
            </div>
            <div>
              <a class="text-blue-500" :href="'mailto:' + item.valore" v-if="item.idTipoContatto == 1">{{ item.valore
}}</a>
...
function startVoip(valore) {
  console.log('|| `  ..--** - voip - **--..  ` ||')
  axios.get('https://aldebaran.cluana.com/PBX/seri_endpoint.php?service=call_request&token=7dfe63ebb0e55161b31739d3ecddeb75&ext=' + store.getters.loggedUser.numero_voispeed + '&number=' + valore).then((res) => res)
}
...
function contattoAction(contatto) {
  console.log("🚀 ~ file: CrmUser.vue:136 ~ contattoAction ~ contatto", contatto)
