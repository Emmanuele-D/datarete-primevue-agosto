    ...
    <div class="form-signin">
      <form @submit.prevent="login">
        ...
        <span class="w-100 p-float-label mb-5">
          <InputText id="username" class="w-100" type="text" v-model="username" placeholder="Nome Utente" />
        </span>
        <span class="w-100 p-float-label mb-5">
          <Password id="password" class="w-100" v-model="password" :feedback="false" placeholder="Password"
            toggleMask />
        </span>
        <Button label="Entra" class="w-100" type="submit" />
      </form>
      <span v-if="$store.getters.authStatus == 'error'" style="margin-top: 1rem; color:tomato">Problema con
        l'autentizacione<br>Prova a reinserire i dati</span>
    </div>
  </div>
...
export default {
  ...
  methods: {
    login: function () {
      ...
      this.$store.dispatch(AUTH_REQUEST, authObject)
        .then((res) => {
          console.log(" STORE GETTER STATUS  ", store.getters.authStatus)
          this.authStatus = store.getters.authStatus
          if (res.data.ActionRequired != null && res.data.ActionRequired != "") {
