<template>
  ...
  <div class="wrapper">
          ...
          <AccordionTab header="Filtri" class="w-full">
            <div class="flex flex-wrap w-full gap-4 align-items-center">
              ...
              <div class="flex flex-column  flex-grow mb-4">
                ...
                <InputText type="text" v-model="filtro.codice_fiscale"></InputText>
              </div>
              <div class="flex flex-column  flex-grow mb-4">
                <label>Telefono</label>
                <InputText type="text" v-model="filtro.numero_telefono"></InputText>
              </div>
              <Divider></Divider>
              <div class="flex flex-column col-md-3 col-12 flex-grow mb-4">
                ...
                <MultiSelect :filter="true" :options="statoOptions" optionLabel="nome" optionValue="id"
                  v-model="filtro.stato" @change="getSubStatoOptions(filtro.stato[filtro.stato.length - 1])">
                </MultiSelect>
              </div>
              <div class="flex flex-column col-md-3 col-12 flex-grow mb-4">
                <label>Sottostato</label>

                <MultiSelect :filter="true" :options="sottostatoOptions" optionLabel="nome" optionValue="id"
                  v-model="filtro.sottostato">
                </MultiSelect>
              </div>
              <div class="flex flex-column col-md-3 col-12 flex-grow mb-4">
                <label>Origine</label>
                <MultiSelect :filter="true" :options="origineOptions" optionLabel="nome" optionValue="id"
                  v-model="filtro.origine">
                </MultiSelect>
              </div>
              <div class="flex flex-column col-md-3 col-12 flex-grow mb-4">
                <label>Commerciale</label>
                <MultiSelect :filter="true" :options="usersOptions" optionLabel="text" optionValue="value"
              ...
              <div class="flex flex-column  flex-grow mb-4">
                ...
                <Calendar inputId="range" selectionMode="range" :manualInput="false" v-model="filtro.data_creazione" />
              </div>
              <div class="flex flex-column  flex-grow mb-4">
                <label>Data modifica</label>
                <Calendar inputId="range" selectionMode="range" :manualInput="false" v-model="filtro.data_modifica" />
              </div>
              <div class="flex flex-column  flex-grow mb-4">
                <label>Data prossimo rinnovo</label>
                <Calendar inputId="range" selectionMode="range" :manualInput="false"
              ...
              <div class="flex w-full justify-content-between">
                <div>
                  Numero di risultati: {{ config.totalRecords }}
                </div>
                <div>
                  <Button :loading="loading" @click="resetFilter" label="Resetta i filtri" class="mr-2"></Button>
                  <Button :loading="loading" @click="getTableItems" label="Filtra"></Button>
                </div>
              </div>
            </div>
          </AccordionTab>
    ...
    <TableBuilder @onSort="onSort($event)" @onPage="onPage($event)" @event_ShowSidebar_eye="goToCrm"
      @event_ShowSidebar_edit="showEditAnagrafica" @event_ShowSidebar_modifica="null"
      @event_ShowSidebar_nuovoAppuntamento="$router.push('/eventi')" @event_ShowSidebar_creaPratica="null"
      @event_ShowSidebar_elimina="confirmDelete" @event_Menubar_Inserisci="showSidebarEventi" :data="tableItems"
      :headersConfig="columns" :config="config" :showAzioni="true">
    </TableBuilder>
...
function getTableItems() {
  ...
  const service = new AxiosService('Anagrafiche/RetailLight?serverPagination=true?fromIndex=' + lazyParams.value.fromIndex + '?takeElements=' + lazyParams.value.takeElements + '?type=' + route.name)

  service.postCustomEndpoint('Anagrafiche/RetailLight?serverPagination=true&fromIndex=' + lazyParams.value.fromIndex + '&takeElements=' + lazyParams.value.takeElements + '&type=' + route.name, '', filtro.value,)
    .then(res => {
      res.forEach(element => {
...
function getUsersOptions() {
  ...
  service.read()
    .then(res => usersOptions.value = res)
}
...
function getStatoOptions() {
  ...
  service.read()
    .then(res => statoOptions.value = res)
}
