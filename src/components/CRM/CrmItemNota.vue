<template>
  ...
  <div v-if="viewMode == 'view'">
    ...
    <div v-if="item.allegato" class="grid">
      <div class="col-12">
        <a :href="'https://posadas-core.datarete.cloud/'+item.allegato" target="_blank">
    ...
    <div class="grid mt-4">
      ...
      <div v-if="store.state.user" class="col-4 flex justify-content-end align-items-end">
        <Button v-if="store.state.user.user.id == item.id_utente" @click="switchToEditMode" icon="pi pi-pencil"
          class="p-button-rounded mr-2"></Button>
        <Button v-if="store.state.user.user.id == item.id_utente" @click="confirmDeleteItem(item)" icon="pi pi-trash"
  ...
  <div v-if="viewMode == 'edit'">
    <CrmPostPutNota @reloadFeed="reloadFeed" :item="item" @switchToView="switchToWiew"></CrmPostPutNota>
  </div>
