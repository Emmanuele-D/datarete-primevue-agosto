<template>
  ...
  <div class="flex flex-column col-12 mb-4">
    ...
    <FileUpload @uploader="uploadFile" mode="basic" uploadIcon="pi pi-paperclip" name="demo[]" :customUpload="true"
      :previewWidth="50" :maxFileSize="1000000" :chooseLabel="tmpItem.urlFile ? fileName(urlFile) : 'Aggiungi immagine'"
      :auto='true' />
  </div>
  ...
  <div class="w-100 flex justify-content-end align-items-end col-12 mb-4 col-md-2 mt-4">
...
function uploadFile(ev) {
  ...
  for (let i = 0; i < ev.files.length; i++) {
    ...
    service.postCustomEndpoint('Upload?type=' + 'ImgSaleRiunione', '', formData)
      .then(res => {
        console.log("🚀 ~ file: sbGestioneSale.vue ~ line 101 ~ uploadFile ~ res", res)
        tmpItem.value.immagine = 'https://posadas-core.datarete.cloud/' + res
      });
  }
