<template>
  ...
  <h1 class="mb-4">Nuova Circolare</h1>

  <div class="grid w-full">
    <div class="flex flex-column col-12  mb-4">
      <label for="titolo">Titolo</label>
      <InputText id="titolo" type="text" v-model="newCircolare.titolo"></InputText>
    </div>
    <div class="flex flex-column col-12  mb-4">
      <label for="argomento">Argomento</label>
      <InputText id="argomento" type="text" v-model="newCircolare.argomento"></InputText>
    </div>
    <div class="flex flex-column col-12 col-lg-4 mb-4">
      <label for="priorita">Priorità</label>
      <Dropdown id="priorita" v-model="newCircolare.priorita"
    ...
    <div class="flex flex-column col-12 col-lg-4 mb-4">
      ...
      <MultiSelect id="livelli" v-model="newCircolare.livelli" :options="livelliOptions" optionLabel="nome"
        optionValue="id" />
    </div>
    <div class="flex flex-column col-12 col-lg-4 mb-4">
      <label for="livelli">Sedi</label>
      <MultiSelect id="livelli" v-model="newCircolare.sedi" :options="sediOptions" optionLabel="nome"
        optionValue="id" />
    </div>
  ...
  <FileUpload @uploader="uploadFile" mode="basic" uploadIcon="pi pi-paperclip" name="demo[]" :customUpload="true"
    ...
    :chooseLabel="newCircolare.urlFile ? fileName(newCircolare.urlFile) : 'Allega File Circolare'" :auto='true' />
...
function inviaCircolare() {
  ...
  servicePOST.postCustomEndpoint('Circolari/AddCircolare', '', newCircolare.value)
    .then(res => {
      if (res) {
        toast.add({ severity: 'success', summary: 'Nuova Circolare Inviata', detail: props.sidebarData.event.nome, life: 3000 });
...
const serviceLivelliGET = new AxiosService('Options/LoginLevels')
function getLivelli() {
