  ...
  <div v-else class="form-container grid">
    <div class="mb-4 flex justify-content-center w-full ">
      <div class="avatar-container">
        ...
        <Avatar v-if="tmpUser.imgprofilo" :image="tmpUser.imgprofilo" size="xlarge" shape="circle" />
    ...
    <div class="flex flex-column col-12 col-md-6 mb-2">
      ...
      <InputText :class="{ 'red-border': saveDisabled }" id="nome" type="text" v-model="tmpUser.nome">
      </InputText>
    </div>
    <div class="flex flex-column col-12 col-md-6 mb-2">
    ...
    <div class="flex flex-column col-12 col-md-6 mb-2">
      ...
      <InputText id="cellulare" type="text" v-model="tmpUser.cellulare"></InputText>
    </div>
    <div class="flex flex-column col-12 col-md-6 mb-2">
      <label for="datanascita">Data di nascita</label>
      <Calendar id="datanascita" type="text" v-model="tmpUser.datanascita"></Calendar>
    </div>
    <div class="flex flex-column col-12 col-md-6 mb-2">
      <label for="luogonascita">Luogo di Nascita</label>
      <InputText id="luogonascita" type="text" v-model="tmpUser.luogonascita"></InputText>
    </div>
    <div class="flex flex-column col-12 col-md-6 mb-2">
      <label for="codicefiscale">Codice Fiscale</label>
      <InputText id="codicefiscale" type="text" v-model="tmpUser.codicefiscale"></InputText>
    </div>
    <div class="flex flex-column col-12 col-md-6 mb-2">
      <label for="partitaiva">Partita IVA</label>
      <InputText id="partitaiva" type="text" v-model="tmpUser.partitaiva"></InputText>
    </div>
    <div class="flex flex-column col-12 col-md-6 mb-2">
      <label for="username">Username</label>
      <InputText :class="{ 'red-border': saveDisabled }" id="username" type="text" v-model="tmpUser.username">
      </InputText>
    </div>
    <div class="flex flex-column col-12 col-md-6 mb-2">
    ...
    <div class="  flex flex-column col-12 col-md-6 mb-2">
      ...
      <InputText id="ruolo" type="text" v-model="tmpUser.ruolo">
      </InputText>
    </div>
    <div class="  flex flex-column col-12 col-md-6 mb-2">
  ...
  if (!isSending.value) {
    ...
    service.postCustomEndpoint('Upload?type=' + 'UserAvatars', '', formData)
      .then(res => {
        ...
        tmpUser.value.imgprofilo = 'https://posadas-core.datarete.cloud/' + res
      })
      .finally(() => {
...
function save() {
  ...
  if (tmpUser.value.id) {
    servicePUT.update(tmpUser.value)
      .then(res => {
        if (res) {
          toast.add({ severity: 'success', summary: 'Nuova Opzione Creata', detail: tmpUser.value.nome, life: 3000 });
    ...
    console.log('POST , ')
    servicePOST.create(tmpUser.value)
      .then(res => {
        if (res) {
          toast.add({ severity: 'success', summary: 'Nuova Opzione Creata', detail: tmpUser.value.nome, life: 3000 });
          emits('event_refreshList')
...
function getProdottiAbilitati() {
  ...
  service.read().then(res => prodottiOptions.value = res).catch(err => console.log(err)).finally(() => loadingProdotti.value = false)
}

const loading = computed(() => {
  if (!loadingProdotti.value && !loadingLoginLevels.value && !loadingSedi.value && !loadingUsers.value) {
    return false
  }
  return true
})
